...
def build_prompt(lab_data: dict) -> str:
    ...
    if lang == "en":
        prompt = (
            ...
            "Next steps:\n"
            "Final conclusion:\n"
            "Under 'Key observations', keep the format short. Use one line for 'Main condition:', one line for 'Short explanation:', add one line for 'Likely cause:' only when a high-probability cause is supported, and add one line for 'Secondary observations:' only if meaningful secondary abnormalities are present.\n"
        ...
        prompt = (
            ...
            "Next steps:\n"
            "Final conclusion:\n"
            "Bajo 'Key observations', mantén un formato corto. Usa una línea para 'Main condition:', una línea para 'Short explanation:', añade una línea para 'Likely cause:' solo cuando exista una causa de alta probabilidad respaldada por el patrón, y añade una línea para 'Secondary observations:' solo si hay anomalías secundarias relevantes.\n"
            "La condición principal debe contener solo el diagnóstico principal o el patrón dominante. La explicación corta debe seguir siendo breve. La línea de causa probable debe ser prudente y solo de alta probabilidad. Las observaciones secundarias deben limitarse a una sola frase corta e incluir solo anomalías adicionales relevantes.\n"
        ...
        prompt = (
            ...
            "Next steps:\n"
            "Final conclusion:\n"
            "Внутри раздела 'Key observations' держи формат коротким. Используй строку 'Main condition:', строку 'Short explanation:', добавляй строку 'Likely cause:' только если паттерн поддерживает причину высокой вероятности, и добавляй строку 'Secondary observations:' только если есть значимые вторичные отклонения.\n"
            "Main condition должен содержать только главный диагноз или доминирующий паттерн. Short explanation должен оставаться коротким. Likely cause должен быть осторожным и только для причин высокой вероятности. Secondary observations должны ограничиваться одной короткой фразой и включать только значимые дополнительные отклонения.\n"
